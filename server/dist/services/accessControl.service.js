...
const initAccessControl = async () => {
    try {
        ...
        if (findRole) {
            ...
            await createRoleSeller(roles_config_1.seller);
...
const createRole = async (roleInfo, org, rolesMatchs, locale = configs_1.env.locale) => {
    if (rolesMatchs) {
        const organizationFound = await organizations_model_1.default.findById(org);
        try {
            ...
            if (organizationFound) {
                throw new HttpException_1.HttpException(409, (0, i18n_1.__)({ phrase: 'Role {{role}} already exists for organization {{organization}}', locale }, { role: roleInfo.name, organization: organizationFound.name }));
            }
            else {
                throw new HttpException_1.HttpException(409, (0, i18n_1.__)({ phrase: 'Could not find organization', locale }));
            }
        }
    }
    else
