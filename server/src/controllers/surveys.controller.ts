...
const removeSurveyById = async (req: RequestWithUser, res: Response, next: NextFunction) => {
    const {_id} = req.body
    try {
        const survey: Survey = await surveysServices.removeSurveyById(_id)
        res.status(200).json({ data: survey, message: 'surveys removed' })
    } catch (error) {
        next(error)
    }
}
