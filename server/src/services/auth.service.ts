...
const signup = async (
    ...
    sendHTMLEmail(
        ...
        generateHTML(path.join(__dirname, `/../emailTemplates/verify.email.template/${locale}.html`), args),
...
const forgotPassword = async (email: string, locale: string = env.locale) => {
    ...
    await sendHTMLEmail(
        ...
        generateHTML(path.join(__dirname, `/../emailTemplates/reset.password.template/${locale}.html`), args),
        { attachments: [{ filename: 'logo.png', path: frontendAsset('assets/logo/logo-mail.jpeg'), cid: 'logo' }] }
    ).catch(err => logger.error(__({ phrase: err.message, locale })))
